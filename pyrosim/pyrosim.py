...
from nndf import NNDF

from linksdf  import LINK_SDF

from linkurdf import LINK_URDF
...
def Get_Joint_Angle(robotID, jointName):

    desiredJointIndex = jointNamesToIndices[jointName]
    jointState = p.getJointState(bodyUniqueId = robotID, jointIndex = desiredJointIndex)
...
def Send_Joint(name,parent,child,type,position,axis):

    joint = JOINT(name,parent,child,type,position,axis)
...
def Send_Touch_Sensor_Neuron(name,linkName):
    ...
    f.write('    <neuron name = "' + str(name) + '" type = "sensor:touch" linkName = "' + linkName + '" />\n')
    
def Send_Proprioceptive_Sensor_Neuron(name, jointName):
    
    f.write('    <neuron name = "' + str(name) + '" type = "sensor:proprioceptive" jointName = "' + jointName + '" />\n')

def Send_Synapse( sourceNeuronName , targetNeuronName , weight , learningRule = [0,0,0,0,0]):

    f.write('    <synapse sourceNeuronName = "' + str(sourceNeuronName) + '" targetNeuronName = "' + str(targetNeuronName) + '" weight = "' + str(weight) + '" learningRule = "' + str(learningRule) + '"" />\n')
